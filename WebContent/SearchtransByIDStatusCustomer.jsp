...
to.setDestwalletid(request.getParameter("walletid"));
lst=to.transactionreportbywalletid();
%>
<%
if(lst.size()>0)
{
...
for(int i=0;i<lst.size();i++)
{
	...
	else if(type.equals("person"))
	{
		if(walletid.equals(lst.get(i).getSrcwalletid()))
		{
			%>
			<th scope="row"><%=lst.get(i).getTransno() %></th>
			<td><img class="img-fluid profilephoto rounded-circle" src="uploads/<%=lst.get(i).getDestwalletid() %>profile.png"></td>
		...
		else if(walletid.equals(lst.get(i).getDestwalletid()))
		{
			...
			%>
			<th scope="row"><%=lst.get(i).getTransno() %></th>
			<td><img class="img-fluid profilephoto rounded-circle" src="uploads/<%=lst.get(i).getSrcwalletid() %>profile.png"></td>
			<td><%=fcw.getName() %></td>
...
{
	%>No Such Transaction Perform with this walletid<br><%
}
}
else
{
